...
const ContestTabs = ({ type }) => {
  ...
  useEffect(() => {
    ...
    if (platformFilter !== "all") {
      filtered = filtered.filter((contest) => contest.platform.toLowerCase() === platformFilter.toLowerCase())
    }

    // Filter bookmarked contests
    if (showBookmarked) {
      filtered = filtered.filter((contest) => bookmarkedContests.includes(contest.id))
    }

    setFilteredContests(filtered)
  }, [type, upcomingContests, pastContests, platformFilter, showBookmarked, bookmarkedContests])
  ...
  const handleAddSolution = async () => {
    ...
    const result = await addSolutionLink(selectedContest.id, selectedContest.platform, youtubeLink)
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 justify-between">
        ...
        <Button variant={showBookmarked ? "default" : "outline"} size="sm" onClick={toggleBookmarkedFilter}>
              ...
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  ...
                  <Button
                    ...
                    className={bookmarkedContests.includes(contest.id) ? "text-yellow-500" : ""}
                  >
                ...
                {type === "past" && (
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="outline" size="sm" onClick={() => setSelectedContest(contest)}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>Add a YouTube solution link for {selectedContest?.name}</DialogDescription>
                      </DialogHeader>
